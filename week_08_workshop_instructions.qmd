...
# Import the CSV file
# Your code here
tb = pd.read_csv("intro_to_quarto/data/india_tuberculosis.csv")
tb
```
...
edu_cost_table = tb.groupby("education").agg(num_pt = ("id", "count"), mean_total_cost = ("total_cost", "mean")).reset_index()
edu_cost_table
```
...
edu_cost_table = edu_cost_table.sort_values(by='mean_total_cost', ascending=False).reset_index(drop=True)
edu_cost_table
```
...
```{python}
# Your code here
px.bar(edu_cost_table, x = "mean_total_cost", y = "education")
```
