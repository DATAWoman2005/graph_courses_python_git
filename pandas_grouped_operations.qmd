...
```{python}
tips.groupby("day").agg(sum_tip = ("tip", "sum"))
```
...
```{python}
weather = weather[["date", "month", "precipitation", "wind", "weather"]] # subsetting back to a new column
```
...
# normalize = True will give the proportion
```

### checking for  information within each month
Grouped version
...
```{python}
smoker_counts = tips.groupby("smoker")["day"].value_counts().reset_index
```
...
```{python}
tips["cumul_total_bill_day"] = tips["total_bill"].cumsum()
```
