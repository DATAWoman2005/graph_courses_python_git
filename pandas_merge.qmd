...
```
```{python}
pd.concat([people, test_info, test_info_multiple], axis=1)
```
...
## Practice Q:Left join TB cases and their continents
...
```{python}
merged_df = pd.merge(tb_cases_children, country_continents, left_on="country", right_on="country.name.en", how="left")
merged_df.drop(columns=['country.name.en'])
```
...
## outer join
```{python}
pd.merge(total_sales, product_feedback, how="outer")
```
