...
```{python}
#| output: false
# Update rows where 'country' equals 'UAE'
...
```{python}
#| output: false
df_merged = df_merged.drop(columns=["country_y", "country"])
...
#| output: false
# Key metrics
num_countries = df_merged['country'].nunique()
avg_happiness = df_merged['percent_happiness'].mean()
avg_mortality = df_merged['mortality_count'].mean()
avg_internet = df_merged['percent_internet_household'].mean()

print(f"Number of Countries: {num_countries}")
print(f"Average Happiness Score: {avg_happiness:.2f}")
print(f"Average Mortality Rate: {avg_mortality:.2f}")
print(f"Average Internet Use: {avg_internet:.2f}")

```

```{python}
#| output: false
df_merged["%"] = df_merged["percent_happiness"]
map_fig = px.choropleth(
  df_merged.query("year == 2021"),
    locations="country_code",
    color="%",
    hover_name="country",
    title="Map of Countries by Happiness Score",
...
::: {.valuebox title="Average Percentage of Happiness" color="primary"}
`{python} str(avg_happiness.round(1))`
:::

::: {.valuebox title="Mortality Rate per 100 people" color="primary"}
